...
class Atk():
    ...
    def draw(self, sec=30, sec1=0, sec_end=20):
        ...
        if self.type == "blaster":
            ...
            if self.sec == 1:
                self.track = change_scale(self.track, 2, 2)
                if self.topl:
...
##########
        ...
        if an % 20 == 0:
            x11, y11 = randint(0, 540), randint(0, 540)
        ...
        for i in at.blasters:
            ind = at.blasters.index(i)
            while at.blasters[ind].draw() == True:
                at.blasters[ind].draw()
                ind -= 1
        player.draw_player()
        pygame.display.flip()
