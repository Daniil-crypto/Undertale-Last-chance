...
class Player():
    ...
    def change_color(self):
        for i in range(len(self.new_sides)):
            ...
            if self.new_sides[i] < 25:
                ...
                player1.fill((255, 0, 255, 255 - self.new_sides[i] * 10), special_flags=pygame.BLEND_RGBA_MULT) ###
    ...
    def check_death(self):
        global sec, at, broken_soul, red_soul, break1_s, break2_s, GO
        if self.hp <= 0:
            sec = -2
            self.sec_d += 1
            if self.sec_d == 1:
...
def restart():
    global n, g, name, sec, fps, size, wx1, wy1, w2, h2, screen, player, at, box
    n = 2
    g = 0.2
    name = "CHARA"
    sec = 0
